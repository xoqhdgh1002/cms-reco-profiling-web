...
#-----------------------------------------------------------------------------------------------------------------------------------------------------------
	...
	for workflow in cmssw_yaml[cmssw]['workflow']:#Workflow Loop
		...
		print(
"""<li><strong>{0}({1})</strong><br>
<ul class="submenu">""".format(cmssw,workflow)
		)
...
"""<li><strong>{}</strong>""".format(data_type[step])
			...
			print(
"""<ul>
<li>Time & Memory : <a href="{0}Time_Mem_Summary/{1}{2}.txt" title="getTimeMemSummary">[getTimeMemSummary]</a>""".format(result_address,from_data_path,step)
			)
...
<ul>"""
					...
					for res in [i for i in res_list if step in i]:
						...
						print(
	"""<li><a href="http://cms-reco-profiling.web.cern.ch/cms-reco-profiling/cgi-bin/igprof-navigator/releases/{0}{1}" title="{2} time check">[{3} Profiler igprof{4}]</a><a href="{5}RES/{6}{7}" title="txtLink">[txtLink]</a>""".format(from_cgi_path,cgi_bin,res.split('_')[1].split('.')[0],res.split('_')[1].split('.')[0],ig_number,result_address,from_data_path,res)
						)
				print(
"""</ul>"""
				)
...
</li>""".format(result_address,from_cgi_path,igprof_page,igprof_page.split('_')[0],igprof_page.split('_')[0])
					)
				if len(comp_igprof) != 0:
					print("</ul>")
...
"circles/piechart.php?local=false&dataset="+cmssw+"%2F"+gcc+"%2F"+workflow+"%2F"+step+"_circles&resource=time_real&colours=default&groups=package&threshold=0")
				)
