...
sequential_file = open('/eos/user/b/btae/www/Service_Work/test/cms-reco-profiling-web/test/list/cgi-bin.list','r')
cmssw_list = sequential_file.readlines()

data_path = '/eos/project/c/cmsweb/www/reco-prof/cgi-bin/data/releases'

#for data in ["cpu_endjob","mem_live.1","mem_live.99"]:
			...
			if workflow in yaml_file[cmssw_list[i+1].strip('\n')]["workflow"]:
				...
				for data in [".".join(x.split(".")[:-1]) for x in os.listdir("../comp_igprof/{0}/{1}/{2}".format(cmssw_list[i].strip('\n'),workflow,step))]:
					...
					if os.path.isfile(target_path) and os.path.isfile(refer_path):
						...
						html_name = "/eos/project/c/cmsweb/www/reco-prof/results/comp_igprof/html/{0}/{1}/{2}/{3}.html".format(target_cmssw,workflow,step,data)
